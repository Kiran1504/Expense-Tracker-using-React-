...
export default function Note(props) {
  ...
  // useEffect(()=>{
  //     return CheckforBud(props.explist, props.budList, category.slice(4,));
  // })
  ...
  const createExpense = async (event) => {
    ...
    setamount("");
    setcategory("");
    const {date, cat, amt} = list
    const res = await fetch("/addexpense", {
        method: "POST",
        headers: {
            "Content-Type" : "application/json"
        },
        body: JSON.stringify({date, cat, amt})
    })
  ...
  return (
    <ExpenseProvider value={{expenses}}>
        ...
        <div className="border-2 border-black rounded-xl max-[425px]:m-4">
          <div className="flex justify-center border-b-yellow-700 border-b-2">
            <h3
              ...
              style={
                incomemode
                  ? { backgroundColor: "#000000", color: "#2fc41b" }
                  : { color: "#2fc41b" }
              }
            >
              Income
            </h3>{" "}
            <h3 className=" text-2xl font-semibold lg:m-4 sm:m-3 px-3 m-2 lg:px-6 md:px-2 py-1 lg:py-2">
              |
            </h3>
            <h3
              className=" lg:text-xl font-semibold lg:m-4 sm:m-3 px-3 m-2 lg:px-6 md:px-2 py-1 lg:py-2 rounded-xl"
              ...
              style={
                expensemode
                  ? { backgroundColor: "#000000", color: "rgb(220 38 38)" }
                  : { color: "rgb(220 38 38)" }
              }
            >
              Expense
          ...
          {expensemode ? (
            ...
            <input
              className=" outline-none text-center bg-transparent cursor-pointer hidden"
              name="category"
            />
          )}
          ...
          <div className="my-24 text-center mx-auto ">
            ...
            <form onSubmit={createExpense}>
              ...
              <input
                ...
                name="amount"
                className=" border-2 w-3/4 m-2 text-xl p-2 border-black"
              />
              <button
                type="submit"
                className=" rounded-lg bg-black text-white px-10 py-4 my-4"
              >
                {" "}
                Save{" "}
              </button>
